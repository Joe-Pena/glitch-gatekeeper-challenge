...
function gateKeeper(req, res, next) {
  const userAndPass = queryString.parse(req.get('x-username-and-password'));
  const user = userAndPass.user || undefined;
  const pass = userAndPass.pass || undefined;
  
  req.user = USERS.find((user, index) => user.userName === user && user.password === pass);
  next();
}
